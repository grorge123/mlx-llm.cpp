...
class Encoding {
public:
  Encoding(const std::string& Name, int ExplicitNVocab, const std::string& PatStr,
...
private:
  ...
  mutable std::optional<std::vector<int>> CachedSotSequenceIncludingNotimestamps;
  mutable std::optional<std::vector<int>> CachedNonSpeechTokens;
};

std::unique_ptr<Encoding> getEncoding(const std::string& Name = "gpt2", int NumLanguages = 99);

std::unique_ptr<Tokenizer> getTokenizer(bool Multilingual, int NumLanguages = 99,
                                        const std::optional<std::string>& Language = std::nullopt,
                                        const std::optional<std::string>& Task = std::nullopt);

// Helper function to create a tokenizer for whisper transcription
std::unique_ptr<Tokenizer> createWhisperTokenizer(const std::optional<std::string>& Language = std::nullopt,
                                                  const std::optional<std::string>& Task = std::nullopt);
